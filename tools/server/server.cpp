...
int llama_server(common_params & params, int argc, char ** argv) {
    ...
    if (!is_router_server) {
        ...
        if (params.scheduler == "paged") {
            ...
            SRV_INF("paged scheduler enabled: block_size=%d max_model_len=%d max_num_seqs=%d admission=%s prefix_cache=%s batch_mode=%s prefill_chunk=%d prefill_target=%.1f ms decode_steps=%d\n",
                    params.kv_block_size, params.max_model_len, params.n_parallel_max,
                    params.paged_admission.c_str(), params.kv_prefix_cache ? "on" : "off", params.paged_batch_mode.c_str(), params.paged_prefill_chunk,
