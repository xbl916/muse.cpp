...
int llama_server(common_params & params, int argc, char ** argv) {
    ...
    if (!is_router_server) {
        ...
        if (params.scheduler == "paged") {
            ...
            SRV_INF("paged scheduler enabled: block_size=%d max_model_len=%d max_num_seqs=%d admission=%s prefix_cache=%s sparse_attn_sink=%d sparse_attn_window=%d prefill_chunk=%d prefill_target=%.1f ms decode_steps=%d\n",
                    ...
                    params.paged_attn_sink, params.paged_attn_window, params.paged_prefill_chunk,
