...
| `--log-wall-clock, --no-log-wall-clock` | Use local wall-clock timestamps (`YYYY-MM-DD HH:MM:SS.mmm`) instead of elapsed time<br/>(env: LLAMA_ARG_LOG_WALL_CLOCK) |
...
| `-bs, --backend-sampling` | enable backend sampling (experimental); concurrent top-k chains are batched, and tensor-parallel CUDA uses shard-local top-k when the sampler prefix permits an exact reduction (default: disabled)<br/>(env: LLAMA_ARG_BACKEND_SAMPLING) |
...
| `--reasoning-budget-low N` | thinking token budget selected when reasoning_effort is 'low'; -1 leaves this effort unconfigured (default: -1)<br/>(env: LLAMA_ARG_THINK_BUDGET_LOW) |
| `--reasoning-budget-medium N` | thinking token budget selected when reasoning_effort is 'medium'; -1 leaves this effort unconfigured (default: -1)<br/>(env: LLAMA_ARG_THINK_BUDGET_MEDIUM) |
| `--reasoning-budget-high N` | thinking token budget selected when reasoning_effort is 'high'; -1 leaves this effort unconfigured (default: -1)<br/>(env: LLAMA_ARG_THINK_BUDGET_HIGH) |
| `--reasoning-budget-xhigh N` | thinking token budget selected when reasoning_effort is 'xhigh'; -1 leaves this effort unconfigured (default: -1)<br/>(env: LLAMA_ARG_THINK_BUDGET_XHIGH) |
| `--reasoning-converge-ratio-low N` | fraction of the low reasoning budget at which to lock the plan and begin convergence; -1 leaves it unconfigured<br/>(env: LLAMA_ARG_REASONING_CONVERGE_RATIO_LOW) |
| `--reasoning-converge-tokens-low N` | non-negative enables low-effort deterministic handoff; the value is retained for compatibility; -1 leaves it unconfigured<br/>(env: LLAMA_ARG_REASONING_CONVERGE_TOKENS_LOW) |
| `--reasoning-converge-ratio-medium N` | fraction of the medium reasoning budget at which to lock the plan and begin convergence; -1 leaves it unconfigured<br/>(env: LLAMA_ARG_REASONING_CONVERGE_RATIO_MEDIUM) |
| `--reasoning-converge-tokens-medium N` | non-negative enables medium-effort deterministic handoff; the value is retained for compatibility; -1 leaves it unconfigured<br/>(env: LLAMA_ARG_REASONING_CONVERGE_TOKENS_MEDIUM) |
| `--reasoning-converge-ratio-high N` | fraction of the high reasoning budget at which to lock the plan and begin convergence; -1 leaves it unconfigured<br/>(env: LLAMA_ARG_REASONING_CONVERGE_RATIO_HIGH) |
| `--reasoning-converge-tokens-high N` | non-negative enables high-effort deterministic handoff; the value is retained for compatibility; -1 leaves it unconfigured<br/>(env: LLAMA_ARG_REASONING_CONVERGE_TOKENS_HIGH) |
| `--reasoning-converge-ratio-xhigh N` | fraction of the xhigh reasoning budget at which to lock the plan and begin convergence; -1 leaves it unconfigured<br/>(env: LLAMA_ARG_REASONING_CONVERGE_RATIO_XHIGH) |
| `--reasoning-converge-tokens-xhigh N` | non-negative enables xhigh-effort deterministic handoff; the value is retained for compatibility; -1 leaves it unconfigured<br/>(env: LLAMA_ARG_REASONING_CONVERGE_TOKENS_XHIGH) |
| `--reasoning-handoff-bias-delay-low N` | compatibility option ignored by deterministic two-stage handoff<br/>(env: LLAMA_ARG_REASONING_HANDOFF_BIAS_DELAY_LOW) |
| `--reasoning-handoff-bias-delay-medium N` | compatibility option ignored by deterministic two-stage handoff<br/>(env: LLAMA_ARG_REASONING_HANDOFF_BIAS_DELAY_MEDIUM) |
| `--reasoning-handoff-bias-delay-high N` | compatibility option ignored by deterministic two-stage handoff<br/>(env: LLAMA_ARG_REASONING_HANDOFF_BIAS_DELAY_HIGH) |
| `--reasoning-handoff-bias-delay-xhigh N` | compatibility option ignored by deterministic two-stage handoff<br/>(env: LLAMA_ARG_REASONING_HANDOFF_BIAS_DELAY_XHIGH) |
| `--reasoning-converge-marker TEXT` | optional marker prefixed to the one-shot reasoning handoff prefix (default: none)<br/>(env: LLAMA_ARG_REASONING_CONVERGE_MARKER) |
| `--reasoning-handoff-transition, --reasoning-handoff-prefix, --reasoning-converge-message TEXT` | transition sentence forced immediately before the native reasoning end tag; empty closes reasoning directly<br/>(default: `The reasoning is sufficient. I will now provide the complete final answer based on the decisions above.`)<br/>(env: LLAMA_ARG_REASONING_CONVERGE_MESSAGE) |
| `--reasoning-converge-boundary-tokens N` | maximum tokens to wait for a safe text boundary before deterministic two-stage handoff (default: 96)<br/>(env: LLAMA_ARG_REASONING_CONVERGE_BOUNDARY_TOKENS) |
| `--reasoning-handoff-bias-delay-tokens N` | compatibility option ignored by deterministic two-stage handoff<br/>(env: LLAMA_ARG_REASONING_HANDOFF_BIAS_DELAY_TOKENS) |
| `--reasoning-converge-max-bias N` | compatibility option ignored by deterministic two-stage handoff<br/>(env: LLAMA_ARG_REASONING_CONVERGE_MAX_BIAS) |
| `--reasoning-hard-boundary-tokens N` | maximum tokens to wait for a line boundary before forcing the reasoning end tag (default: 96)<br/>(env: LLAMA_ARG_REASONING_HARD_BOUNDARY_TOKENS) |
| `--reasoning-budget-soft-ratio N` | legacy fraction of a configured thinking budget at which to inject a one-time wrap-up hint; -1 disables, 0 < N < 1 enables<br/>(env: LLAMA_ARG_THINK_BUDGET_SOFT_RATIO) |
| `--reasoning-budget-soft-message MESSAGE` | one-time wrap-up hint injected at the soft reasoning-budget threshold<br/>(env: LLAMA_ARG_THINK_BUDGET_SOFT_MESSAGE) |
| `--reasoning-budget-soft-boundary-tokens N` | maximum tokens to wait for a safe text boundary before injecting the soft reasoning-budget hint (default: 64)<br/>(env: LLAMA_ARG_THINK_BUDGET_SOFT_BOUNDARY_TOKENS) |
| `--reasoning-budget-grace-tokens N` | extra thinking tokens allowed after the budget before forcing the end tag (default: 0)<br/>(env: LLAMA_ARG_THINK_BUDGET_GRACE_TOKENS) |
...
| `--spec-adaptive, --no-spec-adaptive` | adaptively hard-disable MTP for the rest of a request when recent draft acceptance is too low (default: disabled)<br/>(env: LLAMA_ARG_SPEC_ADAPTIVE) |
| `--spec-adaptive-window N` | number of MTP draft attempts per adaptive evaluation window (default: 64)<br/>(env: LLAMA_ARG_SPEC_ADAPTIVE_WINDOW) |
| `--spec-adaptive-min-gen N` | minimum generated tokens before adaptive MTP can switch off (default: 256)<br/>(env: LLAMA_ARG_SPEC_ADAPTIVE_MIN_GEN) |
| `--spec-adaptive-min-context N` | minimum total context tokens before adaptive MTP can switch off (default: 2048)<br/>(env: LLAMA_ARG_SPEC_ADAPTIVE_MIN_CONTEXT) |
| `--spec-adaptive-min-accept P` | minimum accepted/generated MTP draft-token ratio per window (default: 0.65)<br/>(env: LLAMA_ARG_SPEC_ADAPTIVE_MIN_ACCEPT) |
| `--spec-adaptive-max-empty-rate P` | maximum fraction of MTP attempts producing no usable draft per window (default: 0.25)<br/>(env: LLAMA_ARG_SPEC_ADAPTIVE_MAX_EMPTY_RATE) |
| `--spec-adaptive-patience N` | consecutive bad adaptive MTP windows required before hard-off (default: 2)<br/>(env: LLAMA_ARG_SPEC_ADAPTIVE_PATIENCE) |
| `--spec-adaptive-reasoning-hard-off, --no-spec-adaptive-reasoning-hard-off` | allow adaptive MTP hard-off during reasoning; by default only content can trigger hard-off (default: disabled)<br/>(env: LLAMA_ARG_SPEC_ADAPTIVE_REASONING_HARD_OFF) |
| `--spec-acceptance-log-tokens, --mtp-acceptance-log-tokens N` | generated-token interval for per-request MTP acceptance progress logs; 0 disables (default: 2048)<br/>(env: LLAMA_ARG_SPEC_ACCEPTANCE_LOG_TOKENS) |
...
| `--spec-draft-probabilistic, --no-spec-draft-probabilistic` | sample MTP drafts probabilistically and use p/q rejection verification (default: disabled)<br/>(env: LLAMA_ARG_SPEC_DRAFT_PROBABILISTIC) |
...
Adaptive MTP is evaluated independently for each request. Reasoning and content phases are detected from the chat template's tokenized reasoning markers; requests without marker metadata are treated as content. By default, reasoning windows cannot trigger hard-off; the evaluation window and patience counter are reset when the phase changes, so content is evaluated independently. Use `--spec-adaptive-reasoning-hard-off` to allow reasoning to trigger hard-off too. A hard-off happens only after the current speculative verification has completed; streaming, target KV, sampler state, and the response remain uninterrupted. Hard-off batches skip MTP drafting, MTP catch-up, and target hidden-state extraction. The next request starts with MTP enabled again. If it reuses a target prefix whose MTP state became stale after hard-off, the server performs a full prompt prefill before drafting resumes.

While MTP is active, the server emits a per-request acceptance progress line every 2048 generated tokens by default. Each line includes acceptance for the most recent interval, cumulative acceptance for the request, and the recent mean accepted length. Use `--spec-acceptance-log-tokens N` to change the interval or `0` to disable intermediate lines. The final request-wide acceptance report is unchanged.
...
Per-effort reasoning budgets can be configured with `--reasoning-budget-low`, `--reasoning-budget-medium`, `--reasoning-budget-high`, and `--reasoning-budget-xhigh`. Plan locking and convergence are enabled only when the selected effort also has both a `--reasoning-converge-ratio-*` and `--reasoning-converge-tokens-*` value. An explicitly selected but unconfigured effort remains unrestricted. Requests without a selected effort retain the global `--reasoning-budget` fallback, with no automatic convergence unless the request supplies both `reasoning_converge_ratio` and `reasoning_converge_tokens`. A request-level `reasoning_budget_tokens` takes precedence over the effort budget while retaining that effort's convergence settings.

At the configured ratio, the sampler waits up to `--reasoning-converge-boundary-tokens` for a safe paragraph or complete-sentence boundary, rejecting unfinished delimiter/operator lines and fenced-code interiors. It then deterministically emits the configurable transition sentence followed by the model's native reasoning end tag. Generation resumes immediately in the final-answer content phase. This avoids depending on the model to obey an instruction injected into an already-running reasoning stream. The optional marker remains available for compatibility but is empty by default.

`--reasoning-converge-tokens-*`, `--reasoning-handoff-bias-delay-*`, `--reasoning-handoff-bias-delay-tokens`, and `--reasoning-converge-max-bias` are accepted for v37 configuration compatibility. A non-negative effort-specific convergence-token value still enables handoff for that effort, but deterministic two-stage mode does not generate a convergence window or apply end-tag bias. If the hard reasoning budget is reached before the handoff, `--reasoning-hard-boundary-tokens` still controls the final newline wait before the configured hard message and end tag are forced.

The older `reasoning_budget_soft_ratio`, `reasoning_budget_soft_message`, `reasoning_budget_soft_boundary_tokens`, and `reasoning_budget_grace_tokens` controls remain available for compatibility when per-effort convergence is not enabled. When thinking is disabled with `reasoning_effort: "none"` or `chat_template_kwargs: {"enable_thinking": false}`, all reasoning-budget enforcement, policy and marker injection, convergence bias, boundary waits, legacy hints, and realtime reasoning control are disabled regardless of server or request-level budget settings.
