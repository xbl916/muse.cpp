...
static void ggml_cuda_op_bin_bcast(
    ...
    if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        ...
        op()(src0, src1, dst, (const nv_bfloat16 *) src0_dd, (const nv_bfloat16 *) src1_dd,
             (nv_bfloat16 *) dst_dd, stream);
    } else if (src0->type == GGML_TYPE_BF16 && src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_BF16) {
        op()(src0, src1, dst, (const nv_bfloat16 *) src0_dd, (const float *) src1_dd,
             (nv_bfloat16 *) dst_dd, stream);
    } else if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_BF16 && dst->type == GGML_TYPE_F32) {
        op()(src0, src1, dst, (const float *) src0_dd, (const nv_bfloat16 *) src1_dd,
             (float *) dst_dd, stream);
    } else {
